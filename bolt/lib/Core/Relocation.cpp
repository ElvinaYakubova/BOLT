...
uint64_t createValueX86(uint64_t Type, uint64_t TargetAddress, uint64_t PC) {
  switch(Type) {
  default:
...
uint64_t createValueAArch64(uint64_t Type, uint64_t TargetAddress, uint64_t PC) {
  switch(Type) {
  default:
    llvm_unreachable("not supported relocation");
  case ELF::R_AARCH64_ABS32:
...
uint64_t Relocation::getValue(uint64_t Type, uint64_t TargetAddress,
                                 uint64_t PC) {
  if (Arch == Triple::aarch64)
    return createValueAArch64(Type, TargetAddress, PC);
  return createValueX86(Type, TargetAddress, PC);
}
